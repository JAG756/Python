...
├── rag_system.log          # 运行日志
├── file_state.json         # 文件状态记录（MD5、chunk_ids）
├── batch_process.py        # 批量处理脚本
├── benchmark_direct.py     # 压测脚本，使用多线程并发测试RAG系统的问答性能（响应时间等）
├── test_retrieval.py       # 检索测试脚本，用于对比纯向量检索、混合检索、混合检索+Rerank三种模式下的检索结果
├── evaluate_retrieval.py   # 评估脚本，基于eval_queries.json计算Recall@K、Hit Rate@K、MRR@K等指标，对比不同检索模式。
└── bm25_index.pkl          # BM25 索引文件（混合检索用）
...
Q：为什么“什么是机器学习”返回算法列表而不是定义？
A：长文档中定义块容易被算法块淹没。建议启用分层检索（`USE_HIERARCHICAL=True`），系统会先定位到“核心基础”章节，再检索定义。
...
- [ ] 支持更多文档格式（如 Word、Excel、PPT）
- [ ] 接入更高精度的 Embedding 模型（如 `BAAI/bge-large-zh`）
- [ ] 实现基于用户反馈的检索排序优化
- [ ] 添加 API 接口，供其他系统调用

## 已知限制

- 分层检索目前仅支持 Markdown 的一级和二级标题，对于无标题的纯文本文件会当作“全文”处理。
- 重排序模型首次加载需下载约 1.2GB 文件，请确保网络通畅或提前缓存。
- 增量更新时不会自动重建章节索引，若上传带标题的新 Markdown 文件，建议手动点击“更新知识库”或重启程序。


许可证
MIT License

- 感谢 [Hugging Face](https://huggingface.co/) 提供的模型与工具
- 感谢 [LangChain](https://www.langchain.com/) 的文档处理框架
- 感谢 [Gradio](https://gradio.app/) 的快速界面开发